...
function showImage(index) {
  ...
  img.onload = () => {
    ...
    // ✅ ต้องรอ layout เสร็จก่อน — ใช้ getBoundingClientRect แทน clientWidth
  ...
  return new Promise((resolve) => {
    EXIF.getData(file, function () {
      ...
      img.onload = () => {
        ...
        // ✅ หมุนภาพให้ถูกต้องตาม EXIF orientation
        switch (orientation) {
          case 2:
        ...
        // ✅ วาดภาพด้วยขนาด output ที่ scale แล้ว
        if (needSwap) {
          ctx.drawImage(img, 0, 0, outH, outW); // สลับ w/h เพราะหมุน
        } else {
...
async function detectSingleImage(file) {
  try {
    ...
    // ✅ Debug: เปิด console ดูขนาดภาพที่ส่งจริง
    console.log(`[Send] ${file.name} → ${processedFile.size} bytes`);

    // ✅ Debug: แสดงภาพที่ผ่าน preprocess บน canvas ชั่วคราว
    const debugUrl = URL.createObjectURL(processedFile);
    const debugImg = new Image();
    debugImg.onload = () => {
      console.log(`[PreviewSize] ${debugImg.width} x ${debugImg.height}`);
