  ...
  return new Promise((resolve) => {
    EXIF.getData(file, function() {
      ...
      img.onload = () => {
        ...
        // ขนาดจริงหลังหมุน
        const realW = rotated ? origH : origW;
        const realH = rotated ? origW : origH;

        // Scale ให้พอดี MAX_SIZE
        const ratio = Math.min(MAX_SIZE / realW, MAX_SIZE / realH, 1);
        const canvasW = Math.round(realW * ratio);
        const canvasH = Math.round(realH * ratio);
        ...
        ctx.save();
        // แปลง transform ให้ถูกต้องตาม orientation
        if (orientation === 6) {
          ctx.translate(canvasW, 0);
          ctx.rotate(Math.PI / 2);
          ctx.drawImage(img, 0, 0, canvasH, canvasW); // สลับ w/h
        ...
        }, 'image/jpeg', 0.95); // เพิ่ม quality เป็น 0.95
      };
      img.src = url;
    });
  });
...
console.log('Original size:', file.size, img.width, img.height, 'Orientation:', orientation);
