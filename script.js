...
function showImage(index) {
  ...
  img.onload = () => {
    URL.revokeObjectURL(objectUrl); // ✅ คืน memory ทันที
    ...
    if (result && result.boxes) {
      drawBoundingBoxes(ctx, result.boxes, img.width, img.height, ratio);
    }
    ...
    renderThumbs();
    if (result) displayResults(result); else clearResults();
  };
}
...
async function detect() {
  ...
  for (let i = 0; i < imageFiles.length; i++) {
    ...
    document.getElementById('detectProgress').textContent = `ภาพที่ ${i + 1} จาก ${imageFiles.length}`;
    document.getElementById('resStatus').innerText = `⏳ กำลังประมวลผลภาพ ${i + 1}/${imageFiles.length}`;
    ...
    // วาด bounding box บนภาพปัจจุบัน
    showImage(i);
  ...
  btn.disabled = false;
  document.getElementById('detectProgress').textContent = `✓ ตรวจสอบครบ ${imageFiles.length} รูป`;
}
